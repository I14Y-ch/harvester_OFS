...
PROBLEMATIC_DATASETS = [
    "35827595@bundesamt-fur-statistik-bfs",  
    "283779@bundesamt-fur-statistik-bfs",
    "34807971@bundesamt-fur-statistik-bfs", 
    "32159083@bundesamt-fur-statistik-bfs",
    "12827217@bundesamt-fur-statistik-bfs"
]
...
def cleanup_problematic_datasets(previous_ids):
    """Remove problematic datasets from previous_ids to force recreation"""
    cleaned_ids = previous_ids.copy()
    removed_count = 0
    
    for problematic_id in PROBLEMATIC_DATASETS:
        if problematic_id in cleaned_ids:
            del cleaned_ids[problematic_id]
            removed_count += 1
            print(f"Removed problematic dataset: {problematic_id}")
    
    print(f"Removed {removed_count} problematic datasets from tracking")
    return cleaned_ids
...
def main():
    ...
    # Clean up problematic datasets before processing
    previous_ids = cleanup_problematic_datasets(previous_ids)
    ...
    for item in deleted_datasets:
        log += f"\n- {item}"
    log += "\nProblematic datasets removed (will be recreated next run):\n"
    for item in PROBLEMATIC_DATASETS:
        log += f"\n- {item}"
    ...
    print(f"Problematic datasets marked for recreation: {len(PROBLEMATIC_DATASETS)}")
